...
export default function Products({ apiEndpoint }) {
    ...
    useEffect(() => {
        fetch(apiEndpoint)
            ...
            .then((data) => {
                ...
                setDisplayedProducts(sortedProducts.slice(0, 4));
                setLoading(false);
            })
    }, [apiEndpoint]);
    ...
    function loadProducts() {
        ...
        const remainingProducts = filterProducts.slice(currentProducts, currentProducts + 4);
    ...
    return(
        ...
        <div className='load-button'>
            <button id='loadBtn' onClick={loadProducts} disabled={loading || displayedProducts.length === products.length}>Load</button>
        </div>
        </>
    )
}